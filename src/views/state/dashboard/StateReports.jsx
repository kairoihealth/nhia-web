...
const StateReportsPage = () => {
  ...
  const handleDownloadReport = () => {
    ...
    const drawPieChart = (x, y, radius, data) => {
      ...
      chartData.forEach((item, index) => {
        ...
        const colorObj = pieColors.find((c) => c.status === item.status);
        const color = colorObj
          ? colorObj.color
          : pieColors[index % pieColors.length].color;
  ...
  const pieColors = [
    ...
    // Complaint Types
    { status: "Quality of Care", color: "#81C784" },
    { status: "Service Delivery", color: "#64B5F6" },
    { status: "Administrative", color: "#FFD54F" },
    {
      status: "Financial",
      color: "#89fff6",
    },
  ...
  const filteredPieStatus = useMemo(
    () =>
      pieData
        ...
        ?.map((s) => {
          const colorObj = pieColors.find((c) => c.status === s.status);
          return {
            ...s,
